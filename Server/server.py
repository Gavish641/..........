...
class Server:
    ...
    def handle_signup(self, msg, sock):
        if not self.database.check_user_registered(msg[1]):
            ...
            mac_address = self.clients_names[sock][0]
            encrypted_password = eval(msg[2])
            print(encrypted_password)
            key = self.encryption.encryption_keys[sock]
            decrypted_password = self.encryption.decrypt_data(encrypted_password, key)
            if msg[3]:
                self.database.update_other_users_mac_address(mac_address)
                self.database.insert_user(msg[1], decrypted_password, msg[3], mac_address)
            else:
                self.database.insert_user(msg[1], decrypted_password, msg[3], "")
    ...
    def handle_sorting_numbers_game(self, msg, sock):
        if msg[2] == "start":
            ...
            if self.scores.checkUserExists(username):
                ...
                last_decrypted_mean = int(self.encryption.decrypt_data(eval(last_encrypted_mean[0]), eval(last_encrypted_mean[1]), eval(last_encrypted_mean[2]), eval(last_encrypted_mean[3]), self.encryption.encryption_keys[username]))
