...
class UsersDB:
    ...
    def try_login(self, username, password_data, encryption_key):
        ...
        tag = password_data[3]
        decrypted_entered_password = self.encryption.decrypt_data(eval(encrypted_password), eval(salt), eval(nonce), eval(tag), encryption_key)  # Decrypt the provided password
        decrypted_stored_password = self.encryption.decrypt_data(eval(result[0]), eval(result[1]), eval(result[2]), eval(result[3]), encryption_key)  # Retrieve the stored encrypted password
...
class Encryption:
    ...
    def decrypt_data(self, ciphertext, symmetric_key):
        print(symmetric_key, type(symmetric_key))
